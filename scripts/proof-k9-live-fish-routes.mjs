...
const outDir = path.join(root, 'output', 'playwright', 'k9-live-fish-routes');
const requestedPort = Number(process.env.PROOF_PORT || 0);
const fishScenarios = [
  { slug: 'fish-shore-minnow', labelIncludes: 'shore', setup: 'live-current-fish-school' },
  { slug: 'fish-storm-runner', labelIncludes: 'storm', setup: 'live-current-fish-school' },
  { slug: 'fish-cave-shimmer', labelIncludes: 'cave', setup: 'live-sea-cave' },
  { slug: 'creature-driftjelly', labelIncludes: 'tide', setup: 'live-current-fish-school' },
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      return !!world?.fishVisuals
        && !!world?.debugSetLiveFishScenario
    ...
    for (const scenario of fishScenarios) {
      const setup = await page.evaluate((slug) => {
        ...
        return window.__world.debugSetLiveFishScenario(slug);
      }, scenario.slug);
      if (!setup?.ok) throw new Error(`${scenario.slug}: unable to reach live fish route ${JSON.stringify(setup)}`);
      if (setup.setup !== scenario.setup) throw new Error(`${scenario.slug}: unexpected setup path ${JSON.stringify(setup)}`);
      ...
      await page.waitForFunction(({ slug, labelIncludes }) => {
        ...
        const live = window.__world?.fishSchool?.();
        const renderer = proof?.renderer;
        const row = renderer?.kilnFishSkinsBySlug?.[slug];
        const visualLabel = String(proof?.site?.school?.label ?? '').toLowerCase();
        const liveLabel = String(live?.label ?? '').toLowerCase();
        return renderer?.slug === slug
          && (renderer?.active ?? 0) === 1
          && visualLabel.includes(labelIncludes)
          && liveLabel === visualLabel
          && (row?.loaded ?? 0) > 0
      ...
      const proof = await page.evaluate(() => ({
        fishSchool: window.__world.fishSchool(),
      ...
      results.push({
        slug: scenario.slug,
        setup,
        fishSchool: proof.fishSchool,
        site: proof.fishVisuals.site,
        text: proof.text,
        renderer: {
    ...
    if (generatedRequests.length > 0) {
      throw new Error(`K9 live fish proof leaked generated asset requests: ${JSON.stringify(generatedRequests)}`);
    }
    if (consoleErrors.length > 0 || pageErrors.length > 0) {
      throw new Error(`K9 live fish proof had browser errors: ${JSON.stringify({ consoleErrors, pageErrors })}`);
    }
    ...
    const proof = {
      ...
      consoleErrors,
      pageErrors,
      note: 'Five aquatic GLB slugs reached through live currentFishSchool/currentFishVisualSite contexts after debug setup; no synthetic fishVisualOverride is used.',
    };
