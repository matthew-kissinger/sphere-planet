...
const outDir = path.join(root, 'output', 'playwright', 'k3-home-prop-skins');
const requestedPort = Number(process.env.PROOF_PORT || 0);

const K3_PROPS = [
  { item: 'workbench', slug: 'workbench' },
  { item: 'campfire', slug: 'campfire' },
  { item: 'chest', slug: 'chest' },
  { item: 'bedroll', slug: 'bedroll' },
  { item: 'cropPlot', slug: 'crop-plot' },
  { item: 'dryingRack', slug: 'drying-rack' },
  { item: 'weatherVane', slug: 'weather-vane' },
];

const ITEM_BY_SLUG = Object.fromEntries(K3_PROPS.map((entry) => [entry.slug, entry.item]));
...
async function seedK3Props(page) {
  return page.evaluate((props) => {
    ...
    if (!world.save.import(JSON.stringify(reset))) throw new Error('failed to reset K3 prop proof save');
    world.setZoom?.(0.16);

    const used = new Set([world.player.tile]);
    const placed = [];
    const candidates = world.nearbyTiles(5).filter((tile) => tile !== world.player.tile);
    const place = (item) => {
    ...
    for (const prop of props) place(prop.item);

    const staged = JSON.parse(world.save.export());
    for (const structure of staged.structures) {
      if (structure.item === 'campfire') structure.state = { lit: true };
      if (structure.item === 'chest') structure.state = { storage: { wood: 2, rock: 1 } };
      if (structure.item === 'bedroll') structure.state = { home: true };
      if (structure.item === 'cropPlot') structure.state = { crop: 'berries', growth: 3, fertility: 2 };
      if (structure.item === 'dryingRack') structure.state = { preserves: 1 };
      if (structure.item === 'weatherVane') structure.state = { forecastReads: 1, forecastKind: 'storm', forecastLabel: 'storm front', forecastIntensity: 0.9 };
    }
    if (!world.save.import(JSON.stringify(staged))) throw new Error('failed to import staged K3 prop states');
    return {
      placed,
...
function assertK3Renderer(result, label) {
  const renderer = result?.proof?.structures?.renderer;
  if (!renderer) throw new Error(`${label}: missing structure renderer diagnostics`);
  if (renderer.kilnSkinsLoaded !== K3_PROPS.length) throw new Error(`${label}: expected ${K3_PROPS.length} loaded K3 skins, got ${renderer.kilnSkinsLoaded}`);
  if (renderer.kilnSkinsPending !== 0) throw new Error(`${label}: K3 skins still pending ${renderer.kilnSkinsPending}`);
  if (renderer.kilnSkinFallbacks !== 0) throw new Error(`${label}: K3 fallback triggered ${renderer.kilnSkinFallbacks}`);
  for (const { item, slug } of K3_PROPS) {
...
async function runViewport(browser, port, profile) {
  ...
  const setup = await seedK3Props(page);
  ...
  }, K3_PROPS.map((entry) => entry.slug), { timeout: 90000 });
...
try {
  ...
  console.log(JSON.stringify({
    ...
    loaded: results.map((result) => ({
      ...
      slugs: Object.fromEntries(K3_PROPS.map(({ slug }) => [slug, result.proof.structures.renderer.kilnSkinsBySlug[slug]])),
    })),
  }, null, 2));
} finally {
