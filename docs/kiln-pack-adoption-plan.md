...
| K3W wall and house shell contract | Code-authored/procedural wall panels, corners, wall-with-window openings, wall-with-door openings, half walls/rails, roof joins, foundations, and snap sockets; Kiln skins only after the wall contract exists | `src/sim/structures.ts`, `src/render/structures.ts`, `StructureRenderer` | First slice passing: `npm run proof:c6-wall-shells` proves `floorFoundation`, `wallPanel`, and `wallHalfRail` sockets, weather-safe wall coverage, rail/foundation non-enclosure, and shelter weakening when a wall moves out. Remaining proof must add edge sockets, corners, wall-with-door/window panels, roof joins, and shared-scale decorative skins |
...
- K3W has its first procedural/code-owned wall-shell slice. `floorFoundation`, `wallPanel`,
  and `wallHalfRail` are craftable sockets with separate diagnostics from the door/window/roof
  house kit, and `npm run proof:c6-wall-shells` proves a wall-panel room becomes weather-safe
  while moving one full wall drops shelter back to `room boundary` missing. Kiln house-shell
  skins still wait on edge sockets, corners, wall-with-door/window panels, roof joins, and
  multi-piece snapping.
