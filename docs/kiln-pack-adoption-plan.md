...
| K6R native roaming and ecology state | Harmless grazing/fleeing, territorial patrol/telegraph/recover, shore/cave/fish interactions, pathing across adjacent hexes, and animation state selection | Native-life sim plus `NativeLifeRenderer` | Passing first sparse-roaming plus reaction-readability slice: `npm run proof:k6r-roaming` proves visible creatures derive stable roaming motion from deterministic home sites, move between valid nearby hexes, expose walk/idle clip hints and actual current GLB clips, keep current-tile picking, use approved GLB skins, expose state pose roles for `curious`/`flee`/`telegraph`/`lunge`/`recover`, carry short-lived mining/fishing/failed-ward alert sources into diagnostics, report visible hazard telegraphs separately from hidden telegraph meshes, and avoid generated runtime requests |
...
| K9 aquatic life and fish visibility | Generated Kiln single fish bodies: shore minnow, storm runner, cave shimmer, reed fry, plus a single driftjelly body over existing fishing systems | Fishing sim, waterline structures, `FishSchoolRenderer`, `KilnRuntimeAssets` | Passing visual/provenance and live-route slices: corrected singleton fish are promoted to `models/`, fish-school state selects the matching body, the renderer shows up to two animated GLB anchors plus a point school, mixers are distance-gated, `npm run proof:k9-fish-visuals` exercises all five accepted aquatic GLBs through existing `fishSchoolAt` contexts, and `npm run proof:k9-live-fish-routes` proves all five slugs through normal `currentFishSchool()` / `currentFishVisualSite()` route, weather, cave, tide, and reed-water state without `fishVisualOverride`. Both gates record screenshots/text/model requests, reject generated paths, and keep fallback/browser errors at zero |
...
- K3W has its edge-socket procedural/code-owned wall-shell slice. `floorFoundation`, `wallPanel`,
  ...
  seams excluded, proves exterior wall relocation weakens the room, and proves an interior
  seam door is not required for shelter. It also proves a foundation plus three wall edges on
  ...
  wait on arbitrary/broader footprint proof, cleaner beauty/readability captures, and a
  shared-scale wall-shell pack.
...
K1, K2, K3, K4, K5, K6, K6T, K6R, K7, K9, K11, and the serviced plus full-six-edge K3W/C6 wall-shell slice now
...
5. Execute the readiness warning decisions above: seed/drop replacement, LOD/simplify passes
   ...
   good enough.
6. Finish C6 broader footprint and beauty/readability proof before generating house-shell
   art. Current C6 shelter authority now proves a serviced bedroll-centered wall-shell room,
   a full six-edge single-room shell, a connected four-tile foundation-backed outer-perimeter
   room, and topology-safe invalid-edge blocking on degree-5 pentagon tiles. Arbitrary or
   irregular real-world layouts and a clean reviewer-readable house screenshot still need
   proof. After that broader perimeter plus beauty/readability contract passes, generate one
...
8. Deepen G5/K6R native-life behavior on top of the current sparse roaming actors. The
   first state-aware renderer/readout slice is now in: `curious`, `flee`, `telegraph`,
   `lunge`, and `recover` map to visible pose roles/overlays, current GLB clips are
   reported, and short-lived mining noise, fishing splash, and failed-ward alert sources
   reach diagnostics without changing save shape. Next is the real creature-brain pass:
   define saved vs transient FSM boundaries, social/herd behavior, flee/return timing,
   combat/ward consequences, anti-farming cooldowns, and wider device proof.
