...
| G Native life/combat | G0 behavior state machine; G1 harmless depth; G2 hazard variety; G3 ward/stun/flee tool rules; G4 rewards/anti-farming; G5 sparse roaming actors | B3, D3, E4 | Several native families and ward loops exist; native-life targeting and occupied-tile placement ownership now have a proof gate; G5/K6R has a first sparse-roaming slice with home/current/target tiles, leash movement, clip hints, renderer interpolation, and current-tile picking; next G0/G5 work should be a state-aware reaction pass where existing transient `curious`, `flee`, `telegraph`, `lunge`, and `recover` states drive visible pose/overlay differences before any saved creature-brain rewrite |
...
| G5 native roaming actors | Creature sim, renderer, and animation/perf lanes | Done for first sparse-roaming slice | `npm run proof:k6r-roaming` proves deterministic home sites derive visible roaming actors with current/target tiles, walk/idle clip hints, renderer interpolation, current-tile picking, approved GLB skins, and no generated runtime requests; deeper mood, alert, flee/return, and combat states remain open |
...
| G5/K6R | Native life roams through sparse actor state instead of only tile-local animation | Done for first sparse actor slice | Creature sim/renderer/animation lanes | K6T, H5, G1-G3 | R1, R2, R4, R5 | `src/sim/nativeLife.ts`, `src/render/nativeLife.ts`, `src/edit/pick.ts`, `src/main.ts`, `test/nativeLife.test.ts`, `test/nativeLifePick.test.ts`, `test/nativeLifeRenderer.test.ts`, and `npm run proof:k6r-roaming` | Build the richer state machine on top: mood, alert source, flee/return, social behavior, combat telegraphs, anti-farming cooldowns, and broader device proof |
