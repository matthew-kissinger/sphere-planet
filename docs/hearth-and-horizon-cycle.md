...
| C Building/houses | C0 build-mode UX contract; C1 placement/rotation/dismantle polish; C2 shelter/room validation; C3 readable house-kit assets; C4 utility integrations; C5 functional-home playtest | B1, H0 | C5 is proven for the first hearth loop: build a functional home, complete site work, open a walk-under hearth arch, stock a root cellar, and rest into trail focus |
...
| H Avatar/art/assets | H0 authored Soft-Facet model brief; H1 prop/socket catalog; H2 animation coverage; H3 asset readability pass; H4 external asset ledger/probes | A4 | Route-marker glyph dialect is closed for the first H3 slice; H4 now validates the promoted Kiln pack and wires only the manifest-driven waystone skin pilot with procedural fallback |
...
| R1 Sim/save gate | Focused sim tests pass and debug hooks expose durable state without hiding progression in UI-only code | `npm test -- test/structures.test.ts test/navigation.test.ts` covers capped route-adjacent waterline resupply, off-route ignored gear, the no-ready-gear negative control, and route-consumption clearing set trap/net timers without duplicate inventory | Passing for the E4 route-adjacent and arrival-consumption slice | Save import/export compatibility still needs a broader pentagon-site, route-food, and future organic source-consumption regression suite |
| R2 Playability gate | Real player loop changes state without debug-only shortcuts | `npm run proof:hearth-contract` proves the functional-home/site-work loop; `npm run proof:d2-caves` proves First Hearth and Deep Bell terrain spaces; `npm run proof:e4-ecology-route` proves visible Route Slate food prep changes from off-route ignored/missing to expedition-ready after ready route-adjacent trap/net state, imports an active saved route arrival and verifies the eligible trap/net are spent while off-route gear remains ready, and separately places/sets/waits route plus off-route trap/net gear through existing runtime hooks before hauling and arrival-spending only the route pair | Passing for this slice | Future site kinds, modular house-kit snapping, richer food variety, and landscape-device route-food smoke need the same loop proof |
| R3 Cross-device gate | PC/laptop, touch, and gamepad evidence for affected verbs | Hearth-contract proof covers desktop, 1366x720 laptop, tablet touch portrait, phone touch portrait, and gamepad-active profile; D2 proof adds desktop and phone-touch hearth/bell screenshots; E4 proof now covers desktop, laptop-height, tablet touch portrait, phone touch portrait, and synthetic gamepad Route Slate screenshots | Passing for the current affected verbs only | Landscape tablet/phone, real hardware gamepad, and exact future structure-placement/use paths remain later all-verb parity work |
| R4 Asset readability gate | Reviewer can name the gameplay noun and likely verb from screenshots | Route markers expose named cave-anchor/waystone glyph roles; D2 proof now captures separate First Hearth underpass and Deep Bell chamber screenshots instead of relying on JSON-only terrain counts | Passing for route-marker glyph dialect, waystone pilot, and first threshold-space proof | Blind screenshot review and further avatar/cave-anchor/waystone/threshold polish remain open |
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:hearth-contract`, `npm run proof:d2-caves`, and `npm run proof:e4-ecology-route` record screenshots, screenshot/canvas pixels, no page/console errors, and terrain/progression/route-prep/arrival-consumption/organic route/off-route source evidence | Passing for the promoted Kiln pack, first runtime pilot, hearth contract, first D2 threshold-space proof, and E4 route-adjacent plus organic source proof | Production large-chunk warning, SFX loudness/provenance cleanup, landscape-device route-food smoke, and family-specific scale/collision proofs remain known follow-ups |
...
style. The current avatar exposes hand/back sockets, legs, boots, a belt, satchel, rounded
pack, hood/face/scarf cues, sampled SDF blend-shell body and limb meshes, WebGPU vertex
breathing, and squash-and-stretch procedural action beats; shows owned tools on the backpack;
shows selected blocks or structure kits in hand; and maps walking, sprinting, jumping,
swimming, plane mode, mining, chopping, placing, crafting, fishing, farming, cooking,
resting, and discovery to short readable poses. This does not replace the eventual authored
model; it locks the gameplay contract and art direction that the authored model must satisfy.
...
cave anchors. The first richer chest UI is now in as well: `R` opens a compact storage
...
HUD layouts. The next Cycle 2 work is broader relocation rules and
more visible warmth/light effects in the world. The first
