...
- A promoted Kiln asset pack with 66 ready GLBs that should become the default art
...
6. The approved asset pack is the visual target.
   ...
   same one-asset-one-noun discipline. The first bird pack exists in generated quarantine
   and needs viewer review before promotion.
...
| C Building/houses | C0 build-mode UX contract; C1 placement/rotation/dismantle polish; C2 shelter/room validation; C3 readable house-kit assets; C4 utility integrations; C5 functional-home playtest; C6 wall/shell socket contract | B1, H0 | C5 is proven for the first hearth loop; C1 has placement rotation/pack proof; C2 now has a single-room enclosure and comfort-readability contract around the claimed bedroll; C2/C3 has relocation, code-owned house-kit sockets, socket-specific snap-preview silhouettes, and the first normalized Kiln door/window/roof skins as decorative art only; C6 is now the explicit missing wall contract for procedural/code-authored wall panels, wall openings, corners, rails, foundations, roof joins, snap math, collision, and enclosure before any more house-shell GLBs become build truth |
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Open and now explicit | Current door/window/roof GLBs are only decorative skins or inserts; next building slice must define code-owned wall panels, wall-with-window openings, wall-with-door openings, corners, half walls/rails, roof joins, and optional foundations before more craftable house-shell GLBs can ship |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K5 repeated-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming, and K9 fish slices passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k9-fish-visuals`; current evidence proves waystone, door/window/roof skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming, and five fish/driftjelly bodies | Continue with C6 wall/shell sockets, K3 home props, K4 waterline utilities, K11 sky life, K7 wonders, and remaining modular kits |
...
| C6 | A functional house has real wall and shell sockets | Open | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | Current docs only: `docs/kiln-pack-adoption-plan.md` and `docs/kiln-asset-intake.md` define the required wall/shell contract | Define full wall panels, wall openings, corners, rails, roof joins, and optional foundations in code before requesting or wiring more house-shell GLBs |
...
| Generated asset safety lane | H4, R5 | Audit local Kiln drop, generated GLBs, source kit, and secret/presigned URL risk | Accepted | Merged into proof script, `.gitignore`, tool safety check, and intake docs | 66 committed GLBs and 3 unused records now pass the promoted-pack gate; raw drops, `.env.local`, dogfood outputs, and package tarballs are enforced as untracked |
...
| R4 Asset readability gate | Reviewer can name the gameplay noun and likely verb from screenshots | Route markers expose named cave-anchor/waystone glyph roles; D2 proof captures separate First Hearth underpass and Deep Bell chamber screenshots; `npm run proof:character` now captures Soft-Facet Wayfarer desktop/laptop/tablet/phone/gamepad/WebGL screenshots after asserting 46 silhouette/readability parts, 3 prop sockets, 21 action poses, held/stowed prop separation, and plane seated cues; C2 now exposes visible warmth halo, home comfort ring, roof shelter glow, window warm light, and structured comfort diagnostics; C2/C3 exposes door/window/roof socket dimensions, socket-specific snap-preview guides for door openings, window light, and roof caps, and fitted promoted door/window/roof GLB skins with procedural fallback; `npm run proof:kiln-asset-viewer` now captures 8 overview plus 66 single-asset socket-local screenshots against the 5.6 world-unit hex frame; H5 now makes the remaining approved pack the target visual backlog; K1 adds promoted wood/ore pickup GLB skins; K2 adds all 12 promoted domain-resource node skins while keeping code-owned base/glow overlays; K5 adds four promoted tree/shrub skins while keeping chop/drop authority code-owned; K6 adds all nine promoted creature GLB skins while keeping warning/reward overlays and native-life behavior code-owned; K6T proves visible native-life targets and blockers read through interaction diagnostics; K9 adds five promoted aquatic singleton bodies with fish-school anchors and point-school rendering | Passing for route-marker glyph dialect, waystone pilot, first threshold-space proof, procedural avatar normal-distance contract, first shelter comfort readability, code-owned house-kit sockets, first normalized house-kit skins, first snap-preview readability, first socket-specific house-kit preview silhouettes, full-pack alignment-viewer review packet, first approved-pack pickup pair, first approved-pack resource-node family, first approved-pack vegetation family, first approved-pack creature skin family, native-life targetability/readback, and first fish-school visual slice; H5 is active, not passing beyond the wired route/house/pickup/node/tree/creature/fish slugs | Blind human gameplay-context screenshot review, native-life roaming/state-machine depth, full approved-pack adoption, authored model generation/rigging, two-hand prop polish, wall/shell house pieces, remaining modular prop families, sky-life review/wiring, and further cave-anchor/waystone/threshold polish remain open |
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 66 committed GLBs; `npm run proof:kiln-asset-viewer` proves 66 ready alignment records, 8 overview screenshots, 66 per-asset screenshots, zero generated-path requests, and no browser/page errors; `npm run proof:route-markers` proves committed model requests; `npm run proof:k1-resource-drops` proves committed wood/ore pickup model requests, no generated requests, 5 spawned pickup instances on 5 instanced draw calls, desktop/phone screenshots, and collection into inventory; `npm run proof:k2-domain-resources` proves all 12 committed node model requests, no generated requests, 36 revealed node instances on 33 instanced draw calls, desktop/phone screenshots, and zero pending/fallback; `npm run proof:k5-trees` proves all four committed tree model requests, no generated requests, 210 resident tree instances on 11 instanced draw calls, desktop/phone screenshots, zero pending/fallback, near-only sway, and chopped wood drops; `npm run proof:k6-creatures` proves all nine committed creature model requests, no generated requests, required idle/walk clips, active/low-rate/hidden mixer bands, desktop/phone screenshots, zero fallback, and tend/ward gameplay responses; `npm run proof:k7-native-targeting` proves desktop/phone creature-first targeting and occupied-tile blockers; `npm run proof:k9-fish-visuals` proves the promoted cave-fish GLB path with two animated anchors, 32 point sprites, and zero fallback; `npm run proof:character` proves avatar renderer stats, screenshot/canvas pixels, and no page/console errors across six profiles; `npm run proof:hearth-contract`, `npm run proof:d2-caves`, `npm run proof:e4-ecology-route`, `npm run proof:f1-itinerary`, `npm run proof:panel-ownership`, `npm run proof:c1-build-placement`, `npm run proof:c2-room-enclosure`, and `npm run proof:c2-c3-building-snap-grid` record screenshots, screenshot/canvas pixels, no page/console errors, command logs, terrain/progression/route-prep/arrival-consumption/organic route/off-route/build-placement/relocation/enclosure evidence, promoted door/window/roof model responses, fitted-bounds diagnostics, valid/blocked snap-preview screenshots, and desktop/laptop door/window/roof placement-preview guide screenshots | Passing for the promoted Kiln pack, full-pack alignment viewer, first route-marker pilot, first normalized house-kit family, first K1 instanced pickup pair, first K2 instanced resource-node family, first K5 instanced vegetation family, first K6 animated creature-skin family, K6T native targeting, K9 fish visual slice, avatar fallback proof, hearth contract, first D2 threshold-space proof, E4 route-adjacent plus organic source proof, F1 itinerary controls, shared panel ownership, B1 command boundary, C1 build placement, first C2 room enclosure, first C2/C3 snap-grid relocation/preview, and first socket-specific house-kit preview proof | Production large-chunk warning, SFX loudness/provenance cleanup, landscape-device route-food smoke, native-life roaming/state-machine proof, sky-life proof, and remaining family-specific scale/collision proofs remain known follow-ups |
