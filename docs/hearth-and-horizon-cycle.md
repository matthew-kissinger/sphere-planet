...
weakening shelter, and interior seam door relocation not weakening shelter. Do not generate
or wire the shared-scale Kiln house-shell pack as craftable building art until broader
footprints, pentagon-degree edge cases, and cleaner beauty/readability screenshots are
proven; use procedural wall-shells as the code-owned collider/snap/shelter authority. The
current connected-room screenshot is proof-grade, not a beauty-grade house shot.

Night closeout insight, 2026-07-08: do not widen the runtime again before the next
continuation names its node. The immediate polished next slice is **K9.2 fishing cues and
readback**. Two parallel read-only lanes found that K9 already has real sim depth,
committed singleton fish GLBs, live route-state proof, near-only analytic school motion, and
renderer diagnostics, but most of that is hidden in F3/proof JSON until after the player
presses `R`. The next implementation should surface a compact pre-cast cue from the existing
`currentFishSchool()` result, such as `R cast: storm fish run +2 raw fish bait helps`,
without changing catch math, fish visuals, or route rules. Add the cue to the normal HUD,
`__world`, and `render_game_to_text`; then prove it with a focused interaction gate that
presses the real use/cast input, checks the HUD toast and text readback, confirms inventory
deltas or failure reasons, and treats the current K9 live-route screenshots as state and
provenance evidence rather than beauty-grade fish shots.
...
| E Food/farming/fishing | E0 crop/fish tuning; E1 crop variety; E2 traps/nets loop; E3 cooking/preservation effects; E4 ecology-to-route balance; E5 fish singleton bodies/instanced schools/boids | B2, C4, D4 | E4 now has capped route-adjacent staging, arrival-spent waterline sources, and an organic place/set/wait/haul proof path; E5 has runtime visual/provenance, live-route, and first near-only swim-path slices: corrected singleton fish GLBs are promoted, fish schools select shore/storm/cave/reed/tide bodies, near visuals use two animated anchors, point sprites, analytic near-boid flow, and waterline swim-path beads while far groups freeze richer motion, `npm run proof:k9-fish-visuals` proves all five committed aquatic GLBs through existing `fishSchoolAt` contexts, and `npm run proof:k9-live-fish-routes` proves all five through normal `currentFishSchool()` / `currentFishVisualSite()` state after route/weather/cave/domain setup with screenshots/focus crops, text, committed model requests, and zero fallback/errors. Richer food variety, net ecology tuning, player-facing fishing cues, and beauty-grade fishing framing remain |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K10 pickup/drop skins, K2/K3/K4/K5 repeated/static-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming/reaction-readability, K7 cave-anchor/crater/shrine slice, K9 fish visuals/live routes/near swim paths, K11 sky life, and C6 functional serviced plus connected outer-perimeter wall-shell proof passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k10-resource-drops`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k3-home-props`; `npm run proof:k4-utilities`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k7-wonders`; `npm run proof:k9-fish-visuals`; `npm run proof:k9-live-fish-routes`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone and cave-anchor route-marker skins, three skyfall crater skins, all 12 shrine landmark skins, door/window/roof skins, seven home/camp prop skins, seven utility/waterline skins, all 11 exact pickup/drop skins plus a seed alias, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming plus state-aware reaction/readout, five fish/driftjelly bodies through visual/live-route fish-school paths with near-only analytic flow and swim-path beads, four bird bodies, edge-addressed code-owned foundations/walls/integrated openings/corners/rails/roof joins, edge-based shelter coverage, live player traversal collision against wall shells, serviced shelter sleep through a functional wall-shell room, full six-edge single-room perimeter coverage, and first connected foundation-backed four-tile outer-perimeter coverage | Continue with C6 arbitrary/broader footprint proof plus beauty/readability proof, player-facing fishing cues/beauty framing, future ore/resource expansion, shared-scale house-shell skins only after C6 perimeter/readability gates pass, and avatar/equipment authored assets |
| K9.2 | Fishing schools are understandable before and after the player casts | Ready as next runtime slice; night insight closed in docs | Main implementation plus fishing-UX/proof reviewer lanes | E5, H5 | R2, R3, R5 | K9 visual/live-route proofs already prove committed fish bodies, near-only flow, renderer diagnostics, and provenance. Night closeout lanes accepted a smaller next step: a compact cue derived from `currentFishSchool()`, surfaced in HUD/readback and `render_game_to_text`, then proven by a new interaction gate that presses real use/cast input and checks toast plus inventory/failure readback | Implement `currentFishingCue()`/readback, add `proof:k9-fishing-cues`, and defer beauty-grade fish art claims until a manually framed review capture exists |
...
| C6 | A functional house has real wall and shell sockets | Functional serviced single-room, full six-edge single-room, and first connected foundation-backed outer-perimeter proof passing; arbitrary/broader footprints still open | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `ShelterEnclosureReport.coveredBoundaryEdges`; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json`; `output/playwright/c6-wall-shells/wall-shells-six-edge.png`; `output/playwright/c6-wall-shells/wall-shells-multi-room.png` | Next: prove broader/arbitrary connected footprints, pentagon-degree cases, and cleaner beauty/readability; only then generate and fit shared-scale decorative house-shell skins |
...
| K9 fishing UX cue audit | K9.2, E5, R2, R3, R5 | Identify the smallest player-facing fishing improvement after fish GLBs and near-flow proof | Accepted | Deferred to next runtime slice | Add an immediate `R cast` cue from `currentFishSchool()` rather than touching catch math or fish visuals. The cue should name the school, likely raw-fish yield, bait usefulness, and setup failures in compact HUD/readback copy that works on mobile and gamepad layouts |
| K9 cue proof/readout audit | K9.2, J1, R5 | Decide whether to extend existing K9 visual/live-route proofs or add a separate cue gate | Accepted | Deferred to next runtime slice | Add a separate `proof:k9-fishing-cues` lane. It should press real use/cast input, assert HUD toast text, `lastAction`, fish-school label, near-water/dock/setup state, inventory deltas or denial reasons, committed model requests, and no generated-path/fallback regressions. Existing K9 live-route screenshots stay state/provenance evidence, not beauty-grade art proof |
