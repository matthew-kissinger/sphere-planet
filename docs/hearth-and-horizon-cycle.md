...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Edge-socket and traversal-collision slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, edge-addressed occupancy, same-hex center-plus-edge stacking, duplicate-edge blockers, wall/window/corner traversal blockers, passable integrated door edges, live player movement collision proof, stale collision clearing after relocation, and browser proof. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define edge-based shelter coverage, broader room shapes, and shared-scale decorative skins before craftable house shells can be considered polished |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K10 pickup/drop skins, K2/K3/K4/K5 repeated/static-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming, K7 cave-anchor/crater/shrine slice, K9 fish, K11 sky life, and C6 edge-socket/traversal-collision wall-shell slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k10-resource-drops`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k3-home-props`; `npm run proof:k4-utilities`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k7-wonders`; `npm run proof:k9-fish-visuals`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone and cave-anchor route-marker skins, three skyfall crater skins, all 12 shrine landmark skins, door/window/roof skins, seven home/camp prop skins, seven utility/waterline skins, all 11 exact pickup/drop skins plus a seed alias, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming, five fish/driftjelly bodies, four bird bodies, edge-addressed code-owned foundations/walls/integrated openings/corners/rails/roof joins, and live player traversal collision against wall shells | Continue with edge-based shelter coverage, broader room-shape polish, future ore/resource expansion, shared-scale house-shell skins, and avatar/equipment authored assets |
...
| C6 | A functional house has real wall and shell sockets | Edge-socket and traversal-collision slice passing | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Add edge-based shelter coverage, broader room shapes, and shared-scale decorative skins after proof |
...
| K5 tree instancing audit | H5, R4, R5 | Confirm the safest path for promoted tree/shrub GLBs without turning GLB triangles into gameplay authority | Accepted | Merged into `KilnRuntimeAssets`, `TreeAssetRenderer`, streamer fallback control, tests, proof script, and docs | Sidecar audit confirmed `Trees` must remain the chop/pick/drop authority; the accepted path mirrors resident chunks, uses material-merged instanced batches, keeps procedural trees active until all skins load, and distance-gates cosmetic sway |
