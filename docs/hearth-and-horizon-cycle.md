...
- A promoted Kiln Drop 1 asset pack with 61 ready GLBs that should become the default art
...
| C Building/houses | C0 build-mode UX contract; C1 placement/rotation/dismantle polish; C2 shelter/room validation; C3 readable house-kit assets; C4 utility integrations; C5 functional-home playtest; C6 wall/shell socket contract | B1, H0 | C5 is proven for the first hearth loop; C1 has placement rotation/pack proof; C2 now has a single-room enclosure and comfort-readability contract around the claimed bedroll; C2/C3 has relocation, code-owned house-kit sockets, socket-specific snap-preview silhouettes, and the first normalized Kiln door/window/roof skins as decorative art only; C6 is now the explicit missing wall contract for full wall panels, wall openings, corners, rails, roof joins, and shared-scale house-shell assets |
...
| G5 native roaming actors | Creature sim, renderer, and animation/perf lanes | Open | Future work adds sparse `NativeCreatureActor` state over deterministic native sites: home tile, current/target tile, path progress, facing, state timer, mood/alert source, leash radius, cooldowns, and clip hints; proof must preserve native pick/placement ownership and active mixer budgets |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K5 repeated-family slices, K6 animated-creature skin slice, and K6T targetability slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; current evidence proves waystone, door/window/roof skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, and native creature targeting/build blockers | Continue with C6 wall/shell sockets, K3 home props, K4 waterline utilities, K6R roaming actors, K7 wonders, and remaining modular kits |
...
| G5/K6R | Native life roams through sparse actor state instead of only tile-local animation | Open | Creature sim/renderer/animation lanes | K6T, H5, G1-G3 | R1, R2, R4, R5 | Current docs only: native-life behavior audit accepted into this doc and `docs/kiln-pack-adoption-plan.md` | Add `NativeCreatureActor` types, pure actor advance, hex-neighbor pathing, state-to-clip hints, sparse save overrides, renderer interpolation, tests, and a browser proof preserving mixer budgets |
...
| K6R native roaming audit | G5, H5, R4, R5 | Define the future state-machine path for creatures that should move around | Accepted as future node | Merged into docs only | Deterministic native sites should remain home/claim anchors; nearby active creatures should become sparse `NativeCreatureActor` snapshots with state, target tile, leash/path data, clip hints, and distance-banded simulation/animation cost |
