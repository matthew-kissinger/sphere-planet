...
Night wrap insight, 2026-07-08: C6 should not be treated as true multi-room housing yet.
The current implementation proves a functional serviced wall-shell room around the home
bedroll. `shelterReport` still derives boundary edges as home-tile spokes to the one-ring
boundary, so a real connected multi-room/foundation-expanded house needs an outer-perimeter
room-boundary derivation before second-room exterior walls can become shelter authority. The
next safe C6 slice is either a full six-edge single-room wall-shell proof with cleaner
beauty/readability screenshots, or the source change that computes perimeter edges for all
intended room interior tiles. Do not generate or wire the shared-scale Kiln house-shell pack
as craftable building art until that room-perimeter contract is proven; use procedural
wall-shells as the code-owned collider/snap/shelter authority.
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Edge-socket, edge-shelter, traversal-collision, and serviced-room slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, edge-addressed occupancy, same-hex center-plus-edge stacking, duplicate-edge blockers, edge-based shelter coverage diagnostics, wall/window/corner traversal blockers, passable integrated door edges, live player movement collision proof, stale collision clearing after relocation, and browser proof. The current proof also turns a bedroll-centered wall-shell room into a protected, functional `shelter alive` home with `lived-in` comfort, lit campfire warmth, workbench, chest, and bedroll shelter sleep. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define broader/multi-room shapes, cleaner beauty/readability captures, and shared-scale decorative skins before craftable house shells can be considered polished |
...
| C6 | A functional house has real wall and shell sockets | Functional serviced single-room wall-shell slice passing; true multi-room perimeter still open | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `ShelterEnclosureReport.coveredBoundaryEdges`; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Next: add a full six-edge single-room beauty/readability proof or implement outer-perimeter boundary edges for connected room interior tiles; only then generate and fit shared-scale decorative house-shell skins |
...
code-owned wall-shell room: the bedroll-centered shell reports protected shelter,
functional home, lived-in comfort, workbench/chest/campfire service readiness, and bedroll
rest through shelter sleep. This now claims keyboard, touch, and synthetic-gamepad relocation controls while
