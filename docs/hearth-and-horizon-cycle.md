...
| C Building/houses | C0 build-mode UX contract; C1 placement/rotation/dismantle polish; C2 shelter/room validation; C3 readable house-kit assets; C4 utility integrations; C5 functional-home playtest; C6 wall/shell socket contract | B1, H0 | C5 is proven for the first hearth loop; C1 has placement rotation/pack proof; C2 now has a single-room enclosure and comfort-readability contract around the claimed bedroll; C2/C3 has relocation, code-owned house-kit sockets, socket-specific snap-preview silhouettes, and the first normalized Kiln door/window/roof skins as decorative art only; C6 now has a first procedural/code-owned wall-shell slice for floor foundations, full wall panels, and half rails, with shelter proof that full walls seal and rails/foundations do not. Remaining C6 work is true edge sockets, wall-with-door/window panels, corners, roof joins, multi-piece-per-tile building, collision polish, and shared-scale decorative skins |
...
| H5 approved Kiln pack adoption | Main orchestrator with asset-runtime, batching/perf, animation, UX, and proof lanes | Active goal-level priority; K1 pickups, K2 resource nodes, K5 trees, K6 native-creature skins, K6T native targetability, K9 fish visuals, and K11 sky life passing | `docs/kiln-pack-adoption-plan.md` makes the 70 ready GLBs the target visual backlog; every asset family must become runtime wired, runtime dressing, regenerated, or explicitly superseded; repeated families must use palette/material reuse, instanced or batched geometry, and distance-gated animation before broad rollout; `npm run proof:k1-resource-drops` proves pickup skins; `npm run proof:k2-domain-resources` proves resource nodes; `npm run proof:k5-trees` proves trees/shrubs; `npm run proof:k6-creatures` proves creature skins and animation bands; `npm run proof:k7-native-targeting` proves native creature picks beat terrain mining and occupied native-life tiles block structure placement; `npm run proof:k9-fish-visuals` proves promoted cave-fish GLB anchors and point-school rendering; `npm run proof:k11-sky-life` proves all four promoted bird GLBs as sky-life anchors plus point flocks |
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | First slice passing | `floorFoundation`, `wallPanel`, and `wallHalfRail` are code-owned/craftable sockets with renderer silhouettes and browser proof. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define edge-addressed wall-with-window openings, wall-with-door openings, corners, roof joins, multi-piece-per-tile snapping, and collision before more craftable house-shell GLBs can ship |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K5 repeated-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming, K9 fish, K11 sky life, and first C6 procedural wall-shell slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k9-fish-visuals`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone, door/window/roof skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming, five fish/driftjelly bodies, four bird bodies, and procedural wall/foundation/rail shell sockets | Continue with C6 edge sockets/roof joins, K3 home props, K4 waterline utilities, K7 wonders, and remaining modular kits |
...
| C6 | A functional house has real wall and shell sockets | First slice passing | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, and `wallHalfRail`; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Add true edge sockets, wall-with-door/window panels, corners, roof joins, multi-piece-per-tile snapping/collision, and shared-scale decorative skins after proof |
