...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Edge-socket, edge-shelter, and traversal-collision slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, edge-addressed occupancy, same-hex center-plus-edge stacking, duplicate-edge blockers, edge-based shelter coverage diagnostics, wall/window/corner traversal blockers, passable integrated door edges, live player movement collision proof, stale collision clearing after relocation, and browser proof. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define broader room shapes and shared-scale decorative skins before craftable house shells can be considered polished |
...
| C6 | A functional house has real wall and shell sockets | Edge-socket, edge-shelter, and traversal-collision slice passing | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `ShelterEnclosureReport.coveredBoundaryEdges`; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Add broader room shapes and shared-scale decorative skins after proof |
...
| R5 C6 addendum | Wall-shell edge coverage and collision browser proof | `npm run proof:c6-wall-shells` now records edge-based shelter coverage mode, four-edge weather-safe threshold, true perimeter coverage, covered/door/window boundary edges, wall/window/corner traversal blockers, passable integrated door edges, live player movement hitting a wall blocker, stale corner collision clearing after relocation, screenshots, and canvas pixel probes | Passing for first C6 edge-shelter/traversal-collision slice | Broader room shapes and shared-scale house-shell skins remain |
