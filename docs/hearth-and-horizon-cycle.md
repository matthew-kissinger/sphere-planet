...
| R4 Asset readability gate | Reviewer can name the gameplay noun and likely verb from screenshots | Route markers now expose named cave-anchor/waystone glyph roles, and the waystone GLB pilot preserves those route overlays while skinning only the body shell | Passing for route-marker glyph dialect and waystone pilot | Blind screenshot review and further avatar/cave-anchor/waystone polish remain open |
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs at 5.12 MiB plus 3 unused records with no integrity/security failures; `npm run proof:route-markers` proves committed model requests and no raw generated requests | Passing for the promoted Kiln pack and first runtime pilot | Production large-chunk warning, SFX loudness/provenance cleanup, and future family-specific scale/collision proofs remain known follow-ups |
...
Journal and Route Slate prep surfaces. The next Cycle 4 work is net ecology tuning,
deeper food effects, and more cave/flight expedition food variety.
...
explained. The next Cycle 6 work should deepen those terrain openings into authored-feeling
places: true walk-under arches, larger cave mouths or sealed chambers that belong to
specific pentagons, weather pockets with stronger localized behavior, buildable terraces,
traversal constraints, and a few stranger one-off discoveries that should remain partly
unexplained even after they become useful.
