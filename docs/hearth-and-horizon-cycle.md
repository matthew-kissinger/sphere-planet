...
- A promoted Kiln asset pack with 70 ready GLBs that should become the default art
...
6. The approved asset pack is the visual target.
   ...
   through `SkyLifeRenderer` as animated anchors plus point-flock members.
...
| H4 Kiln asset intake | Main implementation with safety, art-readability, runtime-insertion, and proof reviewer lanes | Done for promoted-pack gate plus waystone and first house-kit family | `docs/kiln-asset-intake.md` plus `npm run proof:kiln-assets` validate 70 committed GLBs and 3 unused manifest records; `npm run proof:route-markers` proves the waystone skin loads from `models/`; `npm run proof:c2-c3-building-snap-grid` proves promoted `door-kit`, `window-frame`, and `roof-bundle` requests, measured runtime source bounds, fitted socket bounds, no raw `generated/` requests, and procedural fallback coverage |
| H5 approved Kiln pack adoption | Main orchestrator with asset-runtime, batching/perf, animation, UX, and proof lanes | Active goal-level priority; K1 pickups, K2 resource nodes, K3 home/camp props, K4 utility/waterline props, K5 trees, K6 native-creature skins, K6T native targetability, K6R roaming, first K7 cave-anchor/crater shells, K9 fish visuals, K11 sky life, and C6 edge-socket wall-shell slice passing | `docs/kiln-pack-adoption-plan.md` makes the 70 ready GLBs the target visual backlog; every asset family must become runtime wired, runtime dressing, regenerated, or explicitly superseded; repeated families must use palette/material reuse, instanced or batched geometry, and distance-gated animation before broad rollout; `npm run proof:k1-resource-drops` proves pickup skins; `npm run proof:k2-domain-resources` proves resource nodes; `npm run proof:k3-home-props` proves seven home/camp structure skins with state overlays preserved; `npm run proof:k4-utilities` proves seven utility/waterline skins with code-owned state overlays and shore blockers preserved; `npm run proof:k5-trees` proves trees/shrubs; `npm run proof:k6-creatures` proves creature skins and animation bands; `npm run proof:k7-native-targeting` proves native creature picks beat terrain mining and occupied native-life tiles block structure placement; `npm run proof:k6r-roaming` proves sparse roaming actors over approved creature skins; `npm run proof:route-markers` proves cave-anchor route-marker GLBs; `npm run proof:k7-wonders` proves all three crater shell GLBs with code-owned overlays preserved; `npm run proof:k9-fish-visuals` proves promoted cave-fish GLB anchors and point-school rendering; `npm run proof:k11-sky-life` proves all four promoted bird GLBs as sky-life anchors plus point flocks; `npm run proof:c6-wall-shells` proves the expanded edge-addressed code-owned wall-shell catalog that future house GLBs must skin rather than replace |
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Edge-socket slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, edge-addressed occupancy, same-hex center-plus-edge stacking, duplicate-edge blockers, and browser proof. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define edge-based shelter coverage, collision polish, broader room shapes, and shared-scale decorative skins before craftable house shells can be considered polished |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K3/K4/K5 repeated/static-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming, first K7 cave-anchor/crater slice, K9 fish, K11 sky life, and C6 edge-socket wall-shell slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k3-home-props`; `npm run proof:k4-utilities`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k7-wonders`; `npm run proof:k9-fish-visuals`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone and cave-anchor route-marker skins, three skyfall crater skins, door/window/roof skins, seven home/camp prop skins, seven utility/waterline skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming, five fish/driftjelly bodies, four bird bodies, and edge-addressed code-owned foundations/walls/integrated openings/corners/rails/roof joins | Continue with K7 shrine landmark skins, edge-based shelter coverage/collision, K10 pickup/ore expansion, remaining modular kits, and avatar/equipment authored assets |
...
| C6 | A functional house has real wall and shell sockets | Edge-socket slice passing | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Add edge-based shelter coverage/collision, broader room shapes, and shared-scale decorative skins after proof |
...
| Generated asset safety lane | H4, R5 | Audit local Kiln drop, generated GLBs, source kit, and secret/presigned URL risk | Accepted | Merged into proof script, `.gitignore`, tool safety check, and intake docs | 70 committed GLBs and 3 unused records now pass the promoted-pack gate; raw drops, `.env.local`, dogfood outputs, and package tarballs are enforced as untracked |
...
| Kiln alignment viewer lane | H5, H4, R4, R5 | Build a visual bench for socket, pivot, scale, and axis review across every approved GLB | Accepted | Merged into asset viewer, proof script, and docs | The viewer renders all 70 ready assets on 5.6 world-unit hex sockets with bounds/orientation warnings and per-slug screenshots before runtime wiring decisions |
| Future Kiln request lane | H5, C6, K9, K10, R4 | Audit what additional generated assets are worth requesting after the approved pack | Accepted | Merged into intake and adoption docs | Request fish/aquatic life, drop skins, future ore nodes after item taxonomy, shared-scale house-shell pieces, and wonder/cave dressing; keep terrain tiles, water, sky, cracks, ribbons, glows, and particles procedural/material |
