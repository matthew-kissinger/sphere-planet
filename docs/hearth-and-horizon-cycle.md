...
broader/irregular footprints and cleaner beauty/readability screenshots are proven; use
procedural wall-shells as the code-owned collider/snap/shelter authority. The current
connected-room screenshot is proof-grade, not a beauty-grade house shot.
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K10 pickup/drop skins, K2/K3/K4/K5 repeated/static-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming/reaction-readability, K7 cave-anchor/crater/shrine slice, K9 fish visuals/live routes/near swim paths/cue readback, K11 sky life, and C6 functional serviced plus connected outer-perimeter plus topology-safe pentagon wall-shell proof passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k10-resource-drops`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k3-home-props`; `npm run proof:k4-utilities`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k7-wonders`; `npm run proof:k9-fish-visuals`; `npm run proof:k9-live-fish-routes`; `npm run proof:k9-fishing-cues`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone and cave-anchor route-marker skins, three skyfall crater skins, all 12 shrine landmark skins, door/window/roof skins, seven home/camp prop skins, seven utility/waterline skins, all 11 exact pickup/drop skins plus a seed alias, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming plus state-aware reaction/readout, five fish/driftjelly bodies through visual/live-route fish-school paths with near-only analytic flow, swim-path beads, and first cast/setup cue readback, four bird bodies, edge-addressed code-owned foundations/walls/integrated openings/corners/rails/roof joins, edge-based shelter coverage, live player traversal collision against wall shells, serviced shelter sleep through a functional wall-shell room, full six-edge single-room perimeter coverage, first connected foundation-backed four-tile outer-perimeter coverage, and invalid-edge blocking on degree-5 pentagon tiles | Continue with C6 arbitrary/broader footprint proof plus beauty/readability proof, richer fishing verbs and beauty-grade fishing framing, future ore/resource expansion, shared-scale house-shell skins only after C6 perimeter/readability gates pass, and avatar/equipment authored assets |
...
| C6 | A functional house has real wall and shell sockets | Functional serviced single-room, full six-edge single-room, first connected foundation-backed outer-perimeter, and topology-safe pentagon invalid-edge proof passing; arbitrary/broader footprints still open | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `ShelterEnclosureReport.coveredBoundaryEdges`; `ShelterEnclosureReport.footprintMode`; `ShelterEnclosureReport.interiorSeamEdges`; `ShelterEnclosureReport.pentagonRoomTiles`; `structurePlacementBlocker` topology checks; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json`; `output/playwright/c6-wall-shells/wall-shells-six-edge.png`; `output/playwright/c6-wall-shells/wall-shells-multi-room.png` | Next: prove broader/arbitrary connected footprints and cleaner beauty/readability; only then generate and fit shared-scale decorative house-shell skins |
...
| R5 C6 addendum | Wall-shell edge coverage, collision, serviced-room, six-edge single-room, connected outer-perimeter, and topology-safe pentagon browser proof | `npm run proof:c6-wall-shells` now records edge-based shelter coverage mode, four-edge weather-safe threshold, full six-edge single-room perimeter coverage, first connected four-tile foundation-backed outer-perimeter coverage, connected-room `footprintMode`, `roomTileCount`, topology-derived `interiorSeamEdges`, covered/door/window boundary edges, interior seam exclusion, exterior wall weakening, interior seam door non-weakening, wall/window/corner traversal blockers, passable integrated door edges, live player movement hitting a wall blocker, stale corner collision clearing after relocation, protected/functional/lived-in shelter readback, workbench/chest/campfire service readiness, bedroll shelter sleep, live pentagon invalid-edge denial, screenshots, and canvas pixel probes | Passing for first C6 functional serviced wall-shell room slice, full six-edge single-room perimeter proof, first connected foundation-backed outer-perimeter proof, and topology-safe degree-5 invalid-edge proof | Arbitrary/broader real-world footprints, cleaner beauty/readability captures, and shared-scale house-shell skins remain |
