...
| H Avatar/art/assets | H0 authored Soft-Facet model brief; H1 prop/socket catalog; H2 animation coverage; H3 asset readability pass; H4 external asset ledger/probes | A4 | Route-marker glyph dialect is closed for the first H3 slice; H4 generated-asset intake now quarantines the Kiln pack behind proof before runtime import |
...
| H4 generated asset intake | Main implementation with safety, art-readability, and UX reviewer lanes | Done for quarantine gate | `docs/kiln-asset-intake.md` plus `npm run proof:kiln-assets` validate 64 local generated GLBs, detect no secrets/presigned URLs, record 85 runtime-readiness warnings, and defer GLB runtime import |
...
| H4 | External generated assets are auditable before runtime import | Done for quarantine gate | Main implementation plus safety/art/UX reviewer lanes | A4, H3 | R4, R5 | `docs/kiln-asset-intake.md`; `scripts/proof-kiln-asset-pack.mjs`; `output/kiln/kiln-asset-pack-proof.json` | Shortlist runtime GLB manifest candidates, then run blind screenshot review and scale/pivot/collision proof before importing any model |
...
| Generated asset safety lane | H4, R5 | Audit local Kiln drop, generated GLBs, source kit, and secret/presigned URL risk | Accepted | Merged into proof script, `.gitignore`, tool safety check, and quarantine docs | 64 GLBs are currently valid and secret-free, `.env.local` contains a live-shaped secret and stays ignored, raw GLBs stay ignored, an earlier orphan UUID asset is now absent from the proofed pack, and `batch.mjs` regen deletes are contained under `KILN_OUT_DIR` |
| Generated asset readability lane | H4, R4 | Rank generated waystone, cave-anchor, home-kit, drop, and creature candidates before runtime import | Accepted | Merged into intake docs; runtime code deferred | First GLB pilot should be one family only, preferably `waystone` or `cave-mouth-dry`; avatar remains code-authored; held/stowed props, marker massing, cave anchors, native-life bodies, and domain resources need debug-off blind screenshots |
| Generated asset UX lane | H4, R3 | Tie GLB intake to survival verbs rather than adding decoration | Accepted | Merged as next DAG edge; runtime code deferred | Controls are implemented, but proof remains partial: add landscape tablet/phone and all-verb survival/build proof before widening new panels, build placement, or combat verbs |
...
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 64 local generated GLBs at 5.76 MiB total with no integrity/security failures and records runtime-readiness warnings before import | Passing for the generated-asset quarantine slice | Production large-chunk warning, SFX loudness/provenance cleanup, and future GLB screenshot/scale/collision proofs remain known follow-ups |
